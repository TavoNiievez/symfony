...
<?php
	...
	ConectarseBaseDatos($link);
			...
			$modificar="delete from  cargo where id= '$sbCodigo'; ";
	
			consultas($modificar);
	
			$consulta="insert into cargo values( '$sbCodigo' ,'$sbNombre')";
			...
			//$ultimo_id = mysqli_insert_id(); 
				...
				$sbCadena .= "alert(  'El Cargo Se ha Registrado Correctamente ' )";
				$sbCadena .= "</script>";
				echo $sbCadena;	
				
				//retorna a la pagina anterior
				Header("Location:../frm/Cargos/Vista.php?id=$sbCodigo");
				
			}
			else
			{	
				$sbCadena =  "<script language='javascript'>";
				$sbCadena .= "alert(  'Error al Registrar El Cargo, Verifique los Datos' )";
				$sbCadena .= "</script>";
				echo $sbCadena;
