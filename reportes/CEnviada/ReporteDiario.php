 ...
 //Conexión con la base de datos
  ConectarseBaseDatos($link);
	 ...
	 $sql = "SELECT le.radicado, le.radicadoinicial, le.remitenteinterno, le.correspondencia, le.destinatario, le.folios, le.fecha, le.hora
				...
				WHERE  le.hora >= '$sbHoraInicial'
				AND le.hora <= '$sbHoraFinal' 
				AND le.fecha >=  '$sbFechaInicial'
  ...
  $pdf->Cell(0,4,'',0,1,'C');
   ...
   $pdf->Cell(0,10,"",0,1,'',0);
    ...
    $pdf->Cell(19,5,'R / INICIAL',1,0,'C');
	$pdf->Cell(19,5,'FECHA',1,0,'C');	 
	$pdf->Cell(16,5,'HORA',1,0,'C');
    $pdf->Cell(70,5,'REMITENTES',1,0,'C');
   ...
   while($row=mysqli_fetch_array($result,MYSQLI_NUM))
    ...
    $pdf->Cell(19,5,$sbRadicadoInicial,1,0,'C');
	$pdf->Cell(19,5,$sbFecha,1,0,'C');	 
	$pdf->Cell(16,5,$sbHora,1,0,'C');
    $pdf->Cell(70,5,$sbRemitente,1,0,'C');
	$pdf->Cell(90,5,$sbCorrespondencia,1,0,'C');	
