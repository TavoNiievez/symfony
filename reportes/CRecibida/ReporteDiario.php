<?php
 ...
 include('../../src/MyFPDF.php');
	...
	//recupero variabkes
	$sbHorario			= $_REQUEST["cmbHorario"];
	$sbFechaInicial		= $_REQUEST["dtFechaInicial"];
	$sbFechaFinal		= $_REQUEST["dtFechaFinal"];
	
	if($sbHorario == 1){
	$sbHoraInicial		= '08:00:00';
	$sbHoraFinal		= '13:00:00';
	}
	
	if($sbHorario == 2){
	$sbHoraInicial		= '13:00:00';
	$sbHoraFinal		= '18:00:00';
	}
 ...
 //Conexión con la base de datos
  ConectarseBaseDatos($link);
	 ...
	 $sql = "SELECT lr.radicado, lr.remitente, lr.entidad, lr.destinatario, lr.folios, lr.fecha, lr.hora
			...
			WHERE lr.hora >= '$sbHoraInicial'
			AND lr.hora <= '$sbHoraFinal'
			AND lr.fecha >= '$sbFechaInicial'
  ...
  $pdf->Cell(0,4,'',0,1,'C');
   ...
   $pdf->Cell(0,10,"",0,1,'',0);
    ...
    $pdf->Cell(19,5,'RADICADO',1,0,'C');
	$pdf->Cell(19,5,'FECHA',1,0,'C');	 
	$pdf->Cell(16,5,'HORA',1,0,'C');
    $pdf->Cell(23,5,'REMITENTES',1,0,'C');
    $pdf->Cell(90,5,'ENTIDAD / DEPENDENCIA',1,0,'C');
	$pdf->Cell(70,5,'DESTINATARIO',1,0,'C');	
	$pdf->Cell(14,5,'FOLIOS',1,0,'C');	 	 
	$pdf->Cell(30,5,'FIRMA',1,1,'C');	 
   ...
   while($row=mysqli_fetch_array($result,MYSQLI_NUM))
	...
	$pdf->Cell(19,5,$sbRadicado,1,0,'C');
	$pdf->Cell(19,5,$sbFecha,1,0,'C');	 
	$pdf->Cell(16,5,$sbHora,1,0,'C');
    $pdf->Cell(23,5,$sbRemitente,1,0,'C');
    $pdf->Cell(90,5,$sbEntidad_dependencia,1,0,'C');
	$pdf->Cell(70,5,$sbDestinatorio,1,0,'C');	
	$pdf->Cell(14,5,$nuFolios,1,0,'C');	
	$pdf->Cell(30,5,'',1,1,'C');
     }//fin del while	
